...
from database import init_db, create_default_users, USE_POSTGRES
...
def _execute_query(query: str, params: tuple = None, fetch: str = 'one'):
    """Execute a query using the appropriate database."""
    if USE_POSTGRES:
        import psycopg2
...
async def create_purchase_endpoint(purchase: PurchaseCreate, current_user: dict = Depends(get_current_user)):
    ...
    return PurchaseResponse(**_dict_from_row(result))


@app.post("/api/purchases/import")
async def import_purchases_endpoint(
    file: UploadFile = File(...),
    current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        count = import_purchases_from_excel(df)
        return {"message": f"Successfully imported {count} purchases"}
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Error importing file: {str(e)}")
...
async def create_sale_endpoint(sale: SaleCreate, current_user: dict = Depends(get_current_user)):
    ...
    return SaleResponse(**_dict_from_row(result))
...
async def import_sales_endpoint(
    ...
    try:
        ...
        count = import_sales_from_excel(df)
        return {"message": f"Successfully imported {count} sales"}
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Error importing file: {str(e)}")
...
async def create_physical_stock_endpoint(stock: PhysicalStockCreate, current_user: dict = Depends(get_current_user)):
    ...
    return PhysicalStockResponse(**_dict_from_row(result))
