...
def get_db_connection():
    """
    Get a raw database connection (for special queries).
    Returns connection object - caller must handle commits/closes.
    """
    if USE_POSTGRES:
        conn = psycopg2.connect(DATABASE_URL, sslmode='require')
        conn.autocommit = True
...
def _create_users_sqlite():
    ...
    if count == 0:
        ...
        logger.info("  - cashier1 / cashier123")
        logger.info("  - cashier2 / cashier123")
